...
| Table 6.2 | Modal rules for K (□T, □F, ◇T, ◇F) | Internal: `apply_box_true_rule`, `apply_box_false_rule`, `apply_diamond_true_rule`, `apply_diamond_false_rule` — generic over an operator pair, so the same bodies serve 𝐆/𝐅 |
...
| Table 6.4 | Tableau systems (K, KT, KD, KB, K4, S4, S5) | [`TABLEAU_K`](@ref), [`TABLEAU_KT`](@ref), [`TABLEAU_KD`](@ref), [`TABLEAU_KB`](@ref), [`TABLEAU_K4`](@ref), [`TABLEAU_S4`](@ref), [`TABLEAU_S5`](@ref) — each derived from its `ModalSystem` via `tableau_rules` (S5 from the KTB45 presentation matching Table 6.4) |
